...
> 정품 벤더 API와의 호출 방식 비교, 확인된 실제 심볼/시그니처, 요청해야 할 스펙 체크리스트는
> [`VENDOR-API-SPEC.md`](./VENDOR-API-SPEC.md) 참고.
...
| `ff_prot_start_exec` | `int (void* buf, uint32 samples_per_ch, uint32 bytes_per_sample, uint32 channels, int32 amb_temp, void* spk_temp, void* spk_exc)` — 7-인자, 검증된 실제 벤더 시그니처와 일치 (`VENDOR-API-SPEC.md` 2.2절) |
