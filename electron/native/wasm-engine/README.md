...
> ⚠️ 이전엔 이 절에서 `VENDOR-API-SPEC.md`(정품 벤더 API 호출 방식 비교/체크리스트 문서)를
> 링크했으나, 그 문서가 "검증됨"으로 문서화했던 7-인자 시그니처는 2026-07-21 새로 확인된
> 벤더 소스(`v_sensing`/`i_sensing` 포함 9-인자)와 어긋나 갱신이 필요해 제거된 상태다.
> 최신 시그니처 근거는 `ff_prot.h` 상단 주석 참고.
