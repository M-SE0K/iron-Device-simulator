#!/usr/bin/env bash
# wasm-preview.sh — ff_prot.c(알고리즘)만 바꼈을 때 전체 재빌드 없이 Electron으로 빠르게 확인한다.
#
# Next.js의 output:"export"는 public/ 을 그대로 out/ 에 복사만 하므로, WASM 산출물만
# 다시 만들어서 out/ 안의 같은 파일을 덮어쓰면 next build 전체를 다시 돌리지 않아도 된다.
# main.js/preload.js(electron-dist/)는 바뀌지 않았다고 가정하고 build:electron:main도 건너뛴다
# — IPC/창 생성 로직을 손봤다면 이 스크립트가 아니라 npm run electron:preview를 써야 한다.
#
