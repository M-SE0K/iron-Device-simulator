/* debug/mute.c — 모든 채널을 완전히 묵음 처리하는 디버그 알고리즘.
 * buf 를 전부 0으로 덮어쓴 뒤, 그 결과(무음) 신호를 기준으로 exc/temp를 추정한다
                       ...
                       const void *i_sensing)
{
    ...
    memset(pcm, 0, (size_t)channels * samples_per_ch * sizeof(int16_t));

    /* 뮤트 상태를 실측 전력에도 반영 — v/i 센싱이 있어도 재생 신호 자체가 무음이므로
     * 앰프 출력단 기준으로는 real_power를 쓰지 않고 buf(0) 기준 근사로 일관되게 둔다. */
    (void)v_sensing;
    (void)i_sensing;
    ...
    for (uint32_t ch = 0; ch < channels; ++ch) {
        ...
        double power = NOMINAL_POWER_W * rms * rms;
