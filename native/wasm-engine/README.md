...
3. **빌드 + 실행** — 리포 루트에서:
   ```bash
   npm run bootstrap    # 클론 직후 원커맨드: 환경 확인 → npm install → build:wasm → dev 서버
   # 또는 개별 실행: npm run build:wasm && npm run dev
   ```
...
build:wasm && npm run dev`). `emcc`가 없으면 `build-wasm.sh`가 Docker(emscripten/emsdk)로
자동 폴백한다.
