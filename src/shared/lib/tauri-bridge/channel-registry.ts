// channel-registry.ts — 리스너가 채널-전역으로 여러 start/stop 세션을 넘어 계속 살아있는
// 구독 방식을 Tauri Channel 위에서 재현한다.
//
// Tauri Channel 인스턴스는 매 invoke(start()) 호출마다 새로 만들어야 하지만, 렌더러가
// onData로 등록한 콜백은 세션 1회(start~stop)에 종속되지 않고 컴포넌트 생명주기를
...
// 등록한다. 그 사이 이미 도착한 첫 청크들을 잃지 않도록 유한 버퍼를 둔다. 최초 구독자
// 1명에게만 플러시한다(그 이후 구독자는 실시간 메시지만 받는다 — 렌더러가 데이터 채널을
// 동시에 여러 곳에서 구독하는 패턴은 없다).
