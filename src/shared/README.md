...
- `window.audioDevice?` / `window.audioCapture?` / `window.audioPlayCapture?` / `window.audioLoopback?` / `window.localFolder?` — Electron 전용 전역. 브라우저/모바일 빌드에서는 `undefined`이므로 feature-detect 필수. `audioDevice`/`audioCapture`의 `deviceUID` 인자는 생략 시 OS 기본 입력 장치를 대상으로 한다.
