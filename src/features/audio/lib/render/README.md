...
- `components/chart/TemperatureChart.tsx` → `chart-window.ts`, `chart-option.ts` 빌더 4종, `DEFAULT_TEMP_WARN`/`DEFAULT_TEMP_DANGER`(markLine)
- `components/chart/ExcursionChart.tsx` → `chart-window.ts`, `chart-option.ts` 빌더 4종
