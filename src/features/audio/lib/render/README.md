...
- `DashboardClient.tsx`의 출력 큐 스케줄러(`RENDER_INTERVAL` 5 ms)가 큐에 쌓인 프레임 bucket을 `detectEvents()` → `coalesceFrames()` 순으로 통과시켜 차트 버퍼(`streamingFrames`)에 넣는다.
...
엔진 frame 수신 → outputQueue(QueuedFrame[]) 적재
  → RENDER_INTERVAL(5ms) 스케줄러가 bucket 추출
