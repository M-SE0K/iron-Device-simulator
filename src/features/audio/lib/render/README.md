...
| `chart-window.ts` | 두 차트 공용의 표시 윈도우 계산 `computeStreamWindow()`와, 지표별로 알고리즘이 다른 Y축 동적 범위 계산 `computeExcursionYRange()`(대칭 패딩)/`computeTemperatureYRange()`(0~100 °C 기본, 10/25/50/100 단위 확장). streaming이면 파일/마이크 구분 없이 항상 전체 누적 프레임을 반환해(0초 원점 고정, 왼쪽 끝 스크롤 방지) `windowSize`로 자르지 않는다 — `windowSize`는 프레임이 없거나 비활성일 때의 fallback slice, 그리고 지금은 호출되지 않는 비streaming(seek) 분기에서만 쓰인다. `ChannelMode`(`"L" \| "R" \| "Both"`) 타입과 두 차트 공용 표시 윈도우 프레임 수 `WINDOW_SIZE`(1000) 정의처 |
| `chart-option.ts` | 두 차트가 동일하게 쓰던 ECharts 옵션 조각 빌더. 축·툴팁 조각(`buildDataZoom()`(inside+slider)/`buildValueTooltip()`(단위·소수점 지정))과 시간 포맷 헬퍼(`timeDecimalsForInterval`/`resolveTimeDecimals`/`buildDynamicTimeFormatter`), series·Y축·옵션 골격(`buildLineSeries()`/`buildValueYAxis()`/`buildAreaGradient()`/`buildBaseChartOption()`)이 있다. `buildDynamicTimeFormatter`는 내부 `floorFixed()`로 표시 자릿수까지 내림(버림) 처리한다 — `toFixed()`의 반올림 대신 `shared/lib/utils.ts`의 `formatTime()`(진행바, `Math.floor` 기반)과 같은 내림 규칙을 맞춰, 초 경계 근처에서 시간축 라벨과 진행바 표시가 최대 1초까지 어긋나 보이던 문제를 없앴다. 시간축(`buildTimeAxis`)·범례(`buildLegend`)는 골격 조립기가 내부에서만 쓰므로 export하지 않는다. 확대(zoom) 시 프레임 점 표시 여부를 판정하는 `shouldShowFrameSymbols()`와 그 상한 상수 `SYMBOL_VISIBLE_MAX`(80)도 여기서 export한다. 지표별로 다른 부분(색·smooth·폭·markLine·grid 좌측 여백·심볼 표시)만 각 차트가 인자로 넘긴다 |
...
- `computeStreamWindow(frames, currentTime, isActive, streaming, audioDuration, windowSize, pick): StreamWindowResult` — 표시 윈도우 계산. streaming이면 파일/마이크 구분 없이 전체 누적 프레임을 그대로 반환(0초 원점 고정, 자르지 않음), 비streaming(seek)은 `currentTime` 위치까지 최대 `windowSize`개(현재 앱에서는 항상 streaming=true라 호출되지 않는 분기). `current`는 헤더 표시용 `[ch0, ch1]` 현재값.
