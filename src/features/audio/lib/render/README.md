...
- `DashboardClient.tsx`의 출력 큐 스케줄러(`RENDER_INTERVAL` 10 ms)가 큐에 쌓인 프레임 bucket을 `detectEvents()` → `coalesceFrames()` 순으로 통과시켜 차트 버퍼(`streamingFrames`)에 넣는다.
...
| `chart-window.ts` | 두 차트 공용의 표시 윈도우 계산 `computeStreamWindow()`와 지표별로 알고리즘이 다른 Y축 동적 범위 계산 `computeExcursionYRange()`(대칭 패딩)/`computeTemperatureYRange()`(0~100 °C 기본, 10/25/50/100 단위 확장). streaming이면 파일/마이크 구분 없이 항상 전체 누적 프레임을 반환해(0초 원점 고정) `windowSize`로 자르지 않는다. 두 차트 공용 표시 윈도우 프레임 수 `WINDOW_SIZE`(1000) 정의처 |
...
- `components/chart/TemperatureChart.tsx` → `chart-window.ts`(`computeStreamWindow`/`computeTemperatureYRange`/`WINDOW_SIZE`), `uplot-option.ts`(`buildTimeAxis`/`buildValueAxis`/`toAlignedData`/`buildAreaFill`/`resolveTimeDecimals`), `uplot-plugins.ts` 3종, `DEFAULT_TEMP_WARN`/`DEFAULT_TEMP_DANGER`(임계선)
...
엔진 frame 수신 → outputQueue(QueuedFrame[]) 적재
  → RENDER_INTERVAL(10ms) 스케줄러가 bucket 추출
...
- `computeStreamWindow(frames, currentTime, isActive, streaming, audioDuration, windowSize, pick): StreamWindowResult` — 표시 윈도우 계산. streaming이면 파일/마이크 구분 없이 전체 누적 프레임을 그대로 반환(0초 원점 고정, 자르지 않음), 비streaming(seek)은 `currentTime` 위치까지 최대 `windowSize`개(현재 앱에서는 항상 streaming=true라 호출되지 않는 분기). `current`는 헤더 표시용 스칼라 현재값, `pick`이 프레임에서 지표를 고른다.
