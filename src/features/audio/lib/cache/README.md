...
- **내부 의존**: `workspace.ts`는 `idb.ts` 위에서, `frame.ts`·`calibration.ts`는 `session-json.ts` 위에서 동작합니다. `audio-blob.ts`는 둘 다 씁니다 — 본문(Blob)은 `idb.ts`, 포인터는 `session-json.ts`로 다룹니다. `frame.ts`와 `workspace.ts`는 `frame-utils.ts`의 `slimAnalysisFrames()`를 함께 씁니다.
- **소비하는 도메인**:
  ...
  - `workspace/` — `WorkspaceContext.tsx`·`RecordsDrawer.tsx`·`hooks/useWorkspaceItems.ts`에 더해 `dashboard/hooks/useWorkspaceSave.ts`도 `workspace.ts`를 씁니다.

```
sessionStorage 계열: frame.ts / calibration.ts / audio-blob.ts(포인터)
    → session-json.ts(readSessionJson/writeSessionJson/removeSessionJson/hasSessionJson) → window.sessionStorage

IndexedDB 계열: audio-blob.ts(본문) / workspace.ts
...
- **`putAudio(file: File)`** / **`getCachedAudio(): Promise<File | null>`** / **`clearAudio()`** — 업로드 오디오 파일의 IndexedDB 저장·복원·삭제. IndexedDB를 못 쓰는 환경에서는 아무 일도 하지 않고 조용히 넘어갑니다. `getCachedAudio()`는 포인터 키가 확실히 없을 때(`hasSessionJson()`이 `false`)만 IndexedDB 본문까지 함께 지웁니다. sessionStorage 접근 불가나 포인터 JSON 파싱 실패면 본문을 남겨둡니다.
...
- **`readSessionJson(key)`** / **`writeSessionJson(key, value)`** / **`removeSessionJson(key)`** / **`hasSessionJson(key): boolean | null`** — sessionStorage 저수준 공용 유틸. `hasSessionJson()`은 SSR·접근 예외에서 `null`을 돌려줘 "없음이 확실함(`false`)"과 "확인 불가(`null`)"를 구분합니다.
...
- 2026-08-19: `session-json.ts`에 `hasSessionJson(key)` 추가, `audio-blob.ts`의 포인터 접근을 `session-json.ts` 헬퍼 경유로 통일(본문은 여전히 `idb.ts`). `getCachedAudio()`는 포인터 부재가 확실할 때만 본문을 지우도록 바뀌었다. 소비 도메인 목록에서 삭제된 `workspace/ChannelViewerOverlay.tsx` 제거. 섹션 3·4·5 부분 갱신 (커밋 범위: 4d86f32..24d1daa)
