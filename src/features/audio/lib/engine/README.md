...
- `RealSensingPair` (`core.ts`) — `{ v: Int16Array; i: Int16Array }`, 각 `samplesPerCh` 길이의 모노 스트림. `local-socket.ts`가 프레임 `byteLength`로 출처를 고른다: 꼬리가 붙어 있으면 4ch 이상 캡처의 전용 센싱 채널(`reframeNativeChunk.ts`의 `SENSING_CHANNEL_INDEX` = ch2/ch3), 없으면 분석 buf의 ch0(V)/ch1(I). 어느 쪽이든 NULL은 넘기지 않는다 — NULL이면 `ff_prot.c`가 PCM RMS 근사로 조용히 대체해버리기 때문이다.
