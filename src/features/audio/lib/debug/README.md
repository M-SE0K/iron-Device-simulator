...
lib/debug/types.ts (leaf, 타입만)
  ├─ player/stream/useAnalysisStream.ts   ← DebugLogEntry 생성(onDebugLog), Partial<StreamDebugInfo> 갱신(onDebugUpdate)
...
- `DebugLogEntry` — 생산: `useAnalysisStream`(수신 시점 필드), 보강: `useRenderTelemetry.handleDebugLog`(렌더 지연 필드). 렌더 지연 3종과 `rttMs`/`freshnessLagMs`는 `number | null`.
