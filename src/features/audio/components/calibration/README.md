...
| `CalibrationDrawer.tsx` | 우측 슬라이딩 드로어. 셸(백드롭·패널·헤더·푸터)은 공용 `shared/components/overlay/SideDrawer`(커스텀 헤더/푸터 슬롯)에 위임하고, `open`/`setOpen`은 `useDrawerState("calibration")`에서 가져온다. hooks/ 4개를 조합하고 로컬 UI 조각(`SelectField`/`NumberField`는 공용 `shared/components/ui/LabeledField` 기반, `DeviceRow`)으로 섹션(THRESHOLD, DEVICE, 연결된 장치)을 구성한다 — 웹 전용 Input/Output Device 섹션은 없고, DEVICE·연결된 장치 섹션 모두 `hasAudioDeviceBridge`(네이티브 브리지)가 있을 때만 그려진다. 헤더에는 닫기 버튼만 두고 제목·트리거는 없다(트리거는 Sidebar 담당). 표시 문구는 전부 이 파일 안에 영문으로 들어 있다. Capture Device 목록의 힌트는 `probed`가 참인 응답에서만 채널 수를 띄운다. Windows 목록은 `--no-probe`라 거짓이고 macOS 목록은 `probed`를 아예 보내지 않으므로, 확정 채널 수는 장치를 고른 뒤 "연결된 장치" 패널에서 보여준다. 드롭다운은 `shared/components/ui/AnimatedSelect`를 쓴다. 열릴 때마다 상태 리셋 + 장치 정보 새로고침을 하는 `[open]` 오케스트레이션 effect를 본체가 소유한다. ESC 닫기는 `useEscapeKey`. |
...
- **In (드로어 개폐)** — `CalibrationDrawer` ← `dashboard/ActiveDrawerContext`의 `useDrawerState("calibration")`. 활성 서랍이 `"calibration"`일 때만 열린다. 여닫는 트리거는 `Sidebar`다.
- **In (에러/성공 피드백)** — `hooks/useCalibrationApply` ← `shared/components/error-popup`의 `useErrorPopup()`. 이전에는 이 도메인이 자체 배너로 처리했는데, 그 몫을 `shared/`의 전역 인프라로 넘겼다. 표시 문구는 다시 각 파일 안의 영문 하드코딩으로 돌아갔다. 2026-08-03 `92fbb5a`가 i18n 배선을 되돌린 결과다.
...
- 2026-08-11: i18n 철수와 서랍 상태 파생 변경을 반영했다. 2026-08-03 `92fbb5a`가 i18n 배선을 통째로 되돌렸다. 그래서 `useLocale()`/`t.calibration.*` 서술은 §3·§4에서 전부 걷어냈다. 드로어 문구·`adjustedNote`·적용 성공/실패 메시지는 다시 각 파일 안의 영문 하드코딩이다. `CalibrationDrawer`가 `useActiveDrawer()` 파생 코드를 직접 갖는 대신 신규 `useDrawerState("calibration")`을 쓰도록 바뀐 것도 §3·§4에 담았다. Capture Device 목록의 힌트가 `probed`가 참일 때만 채널 수를 띄우는 규칙은 §3에 새로 넣었다. 섹션 3·4 부분 갱신 (커밋 범위: 4d86f32..HEAD, 작업 트리 포함)
