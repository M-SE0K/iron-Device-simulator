...
이 도메인은 분석 결과가 어떤 조건에서 나온 값인지를 앱 전체가 하나의 소스로 합의하게 만든다. 조건은 주변온도·샘플레이트·버퍼·캡처 채널 수·장치·온도 임계값이다. 개발자는 이 폴더만 보고도 사용자가 어디서 파라미터를 고치는지, 그 값이 어떤 경로로 엔진·플레이어·차트에 도달하는지 파악한다.
...
- `CalibrationDrawer`를 마운트하는 쪽은 `dashboard/DashboardClient.tsx`고 여닫는 트리거 버튼은 `dashboard/Sidebar.tsx`에 있다. 열림 여부는 `components/` 루트로 올라간 `ActiveDrawerContext`의 `useDrawerState("calibration")`에서 파생하고 그 `ActiveDrawerProvider`도 `app/layout.tsx`가 마운트한다.
- 커밋된 값의 소비처:
  - `dashboard/DashboardClient.tsx` — `ambientTemp`를 `InputParameterValues`로 묶어 엔진 파라미터로, `tempWarn`/`tempDanger`를 이벤트 감지·차트 markLine 임계값으로 읽는다. `channels`는 캡처 헤더가 도착하기 전의 채널 수 폴백(`knownChannelCount`)으로 쓴다.
...
- 스피커 프로파일 계열 필드(`speakerModel`/`ampOutputPower`/`tempBase`/`excAmp`/`tempMult`/`excMult`)와 브라우저 장치 필드(`inputDeviceId`/`inputDeviceLabel`/`outputDeviceId`/`outputDeviceLabel`)는 `CalibrationValues`에서 삭제됐다 — 소스 트리 전체에 참조가 남아 있지 않다.
- 앱이 Tauri 데스크톱 단일 셸로 좁혀지면서(플레인 브라우저 빌드 경로 제거) 브라우저 `navigator.mediaDevices` 기반 Input/Output Device 선택 UI는 드로어에서 완전히 빠졌다. 지금은 `hasAudioDeviceBridge`(네이티브 브리지 존재)가 있을 때만 장치 관련 섹션을 그린다. 파일 재생은 Capture Device의 출력 ch0/ch1로 직접 나가는 구조(play-capture 단일 IOProc)라 Output Device라는 개념 자체가 없다. `outputChannel` 필드는 `CalibrationValues`에 "0" 고정으로 남아 있지만 이를 고르는 UI는 없다. R 채널은 항상 `outputChannel + 1`로 파생된다. 그 장치 출력이 1채널뿐이면 재생 쪽에서 모노로 조용히 폴백한다.

## 3. 파일별 역할

| 파일 | 역할 |
|------|------|
| `CalibrationContext.tsx` | 앱 전역 단일 소스. `CalibrationProvider`, `useCalibration()`만 정의하고 옵션 상수(`SAMPLE_RATE_OPTIONS`/`BUFFER_SIZE_OPTIONS`/`CHANNEL_OPTIONS`/`CALIBRATION_EMPTY`)는 `calibration-options.ts`에서 그대로 재export한다. `CalibrationValues` 인터페이스(8개 필드, 전부 string) 자체는 `features/audio/types.ts`로 옮겨 이 파일은 그 타입을 import만 한다(다른 도메인이 Context를 거치지 않고도 타입을 참조할 수 있도록 단일 소스화). 마운트 후 sessionStorage에서 복원하고(`hydrated` 가드로 기본값 덮어쓰기 방지) 값이 바뀔 때마다 저장한다. |
...
| `CalibrationDrawer.tsx` | 우측 슬라이딩 드로어. 셸(백드롭·패널·헤더·푸터)은 공용 `shared/components/overlay/SideDrawer`(커스텀 헤더/푸터 슬롯)에 위임하고 `open`/`setOpen`은 `components/` 루트 `ActiveDrawerContext`의 `useDrawerState("calibration")`에서 받는다. hooks/ 4개를 조합하고 로컬 UI 조각(`SelectField`/`NumberField`는 공용 `shared/components/ui/LabeledField` 기반, `DeviceRow`)으로 섹션(THRESHOLD, DEVICE, 연결된 장치)을 구성한다 — 웹 전용 Input/Output Device 섹션은 없고 DEVICE·연결된 장치 섹션 모두 `hasAudioDeviceBridge`(네이티브 브리지)가 있을 때만 그려진다. 헤더에는 닫기 버튼만 두고 제목·트리거는 없다(트리거는 Sidebar 담당). 표시 문구는 컴포넌트 안에 영문으로 하드코딩돼 있다. Capture Device 셀렉트의 장치 힌트는 `probed`가 참일 때만 채널 수를 표시하고 기본 장치면 "Default"를 덧붙인다("In Use" 문구는 삭제). 드롭다운은 `shared/components/ui/AnimatedSelect`를 쓴다. 열릴 때마다 상태 리셋 + 장치 정보 새로고침을 하는 `[open]` 오케스트레이션 effect를 본체가 소유한다. ESC 닫기는 `shared/hooks/useGlobalKey`의 `useEscapeKey`. |
| `DeviceSelectField.tsx` | Capture Device 셀렉트가 쓰는 옵션 구성 컴포넌트. 플레이스홀더(`value=""`) 옵션을 앞에 붙인다. 저장된 장치가 현재 목록에 없으면(연결 해제) "연결 안 됨" 힌트를 단 폴백 옵션으로 값을 보존한다. 라벨+컨트롤 레이아웃은 공용 `LabeledField`에 위임하고 `headerRight`/`footnote` 슬롯을 넘긴다. |
| `hooks/useCalibrationDraft.ts` | 드로어 로컬 draft 상태. `open`이 true가 될 때마다 draft를 committed 값으로 동기화하고 부분 갱신 함수 `set(patch)`를 제공한다. |
| `hooks/useCalibrationApply.ts` | "적용" 오케스트레이션. `setValues(draft)`로 Context에 커밋 → 네이티브 브리지가 있으면 `window.audioCapture.start()`/`stop()` capture probe로 실제 SampleRate/BufferFrameSize/Channels를 읽고(TN2321: Buffer는 per-client라 probe로만 확인 가능) `DeviceActualCache`로 sessionStorage에 저장 → `refreshDeviceInfo()`로 패널 갱신. probe 시 채널 수를 `deviceInfo.inputChannels` 이하로 한 번 더 클램프한다. probe 응답의 채널 수(`result.channels`)는 `actual`과 형제 필드로 오므로 합쳐서(`actualWithChannels`) `appliedRuntime.actual`에 저장·표시한다. 브리지가 없으면 커밋 후 드로어를 바로 닫는다. 성공/실패 메시지는 `shared/components/error-popup`의 전역 `useErrorPopup()`(`showSuccess`/`showError`)로 띄운다. |
| `hooks/useDeviceOptionAutoCorrect.ts` | 장치 능력(`query`) 도착 시 DEVICE 섹션 드롭다운 옵션을 그 장치의 지원값으로 재구성한다(SR: `supportedSampleRates`, Buffer: `bufferRange` 안의 정적 목록, Channels: `inputChannels` 이하). draft의 SR/Buffer/Channels가 지원 범위 밖이면 `nearestOption`으로 가장 가까운 지원값에 자동 보정하고 `adjustedNote` 안내 문구(영문 고정 문구 "Auto-adjusted — not supported by this device: …")를 만든다. 조회 중에는 `deviceOptionsLoading`으로 선택을 잠근다. Output Channel 옵션 재구성 로직은 더 이상 없다(위 2번 섹션 참고 — UI 자체가 빠졌다). |
| `hooks/useNativeAudioDevice.ts` | `window.audioDevice`(네이티브 오디오 헬퍼) 조회/열거. `hasAudioDeviceBridge` 감지(마운트 후 — 하이드레이션 불일치 방지), `refreshDeviceInfo(uid?)`(장치 능력 `query`: 현재값·지원 SR 목록·Buffer 범위·입력 채널 수·출력 채널 수), `refreshNativeDevices()`(`list`: UID(Unique Identifier)/이름/채널 수 목록). `DeviceInfo` 타입을 export하며 다른 두 훅이 이를 소비한다. 타입은 `shared/types/native-bridge`에서 가져온다. `outputChannels`가 0이면 그 장치는 입력 전용이라 play-capture(파일 재생)를 할 수 없다. |

## 4. 의존성 및 흐름

도메인 간 데이터 교환(방향 포함):

- **In (조회)** — `hooks/useNativeAudioDevice` → `window.audioDevice`(Tauri IPC, 타입은 `shared/types/native-bridge.d.ts`). 읽기 전용 데이터 페칭이라 실패해도 드롭다운/패널이 비어 보일 뿐 분석에는 영향이 없다.
...
- **Out (영속화)** — `CalibrationContext` → `lib/cache/calibration.ts`(`saveCalibrationCache`/`loadCalibrationCache`, sessionStorage, 탭 수명). `CalibrationValues` 타입의 단일 소스는 이 도메인이 아니라 `features/audio/types.ts`다. 그래서 `CalibrationContext`와 `lib/cache/calibration.ts` 둘 다 거기서 import한다.
...
- **Out (소비자)** — 커밋된 `values`를 `useCalibration()`으로 읽는 외부: `dashboard/DashboardClient`(`ambientTemp` → `InputParameterValues`, `tempWarn`/`tempDanger` 임계값, `channels` 채널 수 폴백), `player/DuplexFilePlayer`·`player/capture/useCaptureSession`(캡처 세션 SR/버퍼/채널/장치 UID). Provider 마운트는 `app/layout.tsx`, 드로어 마운트는 `dashboard/DashboardClient`(트리거는 `dashboard/Sidebar`).
- **In (드로어 개폐)** — `CalibrationDrawer` ← `components/ActiveDrawerContext`(루트)의 `useDrawerState("calibration")`. `active === "calibration"`일 때만 열린다. 여닫는 트리거는 `dashboard/Sidebar`다.
- **In (에러/성공 피드백)** — `hooks/useCalibrationApply` ← `shared/components/error-popup`의 `useErrorPopup()`. 안내 문구는 이 도메인 안에 영문으로 하드코딩돼 있다.
- **In (공용 UI 부품)** — 드롭다운은 `shared/components/ui/AnimatedSelect`, 라벨+컨트롤 레이아웃은 `shared/components/ui/LabeledField`(`SelectField`/`NumberField`/`DeviceSelectField` 공용), 드로어 셸은 `shared/components/overlay/SideDrawer`, ESC 닫기는 `shared/hooks/useGlobalKey`의 `useEscapeKey`에 위임한다.
...
- `CalibrationValues`(`features/audio/types.ts`) — 8개 필드 전부 string: `ambientTemp`/`sampleRate`/`bufferSize`/`channels`/`captureDeviceUID`/`outputChannel`/`tempWarn`/`tempDanger`. 단위: `ambientTemp`/`tempWarn`/`tempDanger` °C, `sampleRate` Hz, `bufferSize` samples/ch, `channels` 캡처 채널 수, `outputChannel` 재생 출력 채널 인덱스("0"=ch0). `""`가 의미를 갖는 필드는 `captureDeviceUID`뿐이다 — OS 기본 입력 장치. 파생 타입 `InputParameterValues`(`Pick<CalibrationValues, "ambientTemp">`)를 대시보드가 엔진 파라미터 전달에 쓴다.
- `CALIBRATION_EMPTY: CalibrationValues`(`calibration-options.ts`, `CalibrationContext.tsx`가 재export) — 기본값(25°C / 48000 Hz / 480 samples/ch / 2ch / `captureDeviceUID` `""` / `outputChannel` `"0"` / WARN 65°C / DANGER 75°C). 드로어 "초기화" 버튼이 draft에 그대로 대입한다.
...
- `CalibrationDrawer()` (default export) — 드로어 본체(트리거는 `dashboard/Sidebar`가 담당). props가 없다 — 과거의 `projectName`/`onApply`는 삭제됐다.
- `DeviceSelectField(props)` (default export) — 장치 셀렉트 공용 컴포넌트. `devices`에 없는 `value`를 `savedLabel` + "연결 안 됨" 힌트로 보존 표시한다.
- `useCalibrationApply(deps): { deviceStatus, appliedRuntime, apply, resetStatus }` — `deviceStatus`는 `"idle" | "applying" | "applied" | "error"`. probe는 마이크가 이미 녹음 중이면 `capture-already-running`으로 실패한다. `appliedRuntime.actual`은 `channels` 필드를 포함한다(probe 응답의 형제 필드를 합친 값). 성공/실패는 `useErrorPopup()`의 전역 팝업으로 띄운다. 과거 반환값이던 `deviceActual`/`deviceError`는 삭제됐다.
...
- BufferFrameSize는 per-client 속성(TN2321)이다. `query`는 장치 기본값만 돌려주며 실제 반영값은 "적용"의 capture probe 결과(`appliedRuntime`)로만 확인한다.
...
- 2026-08-19: `CalibrationValues` 16→8필드 슬림 반영 — `speakerModel`/`ampOutputPower`/프로파일 4필드(`tempBase`/`excAmp`/`tempMult`/`excMult`)/브라우저 장치 4필드가 타입과 `CALIBRATION_EMPTY`에서 삭제됐고 소스 트리 참조도 0건이다. `CalibrationDrawer`는 props 없는 `CalibrationDrawer()`가 됐다(`projectName`/`onApply` 제거). 드로어 개폐는 `components/` 루트로 이동한 `ActiveDrawerContext`의 `useDrawerState("calibration")`로, ESC는 `shared/hooks/useGlobalKey`의 `useEscapeKey`로 바뀌었다. `useCalibrationApply` 반환에서 `deviceActual`/`deviceError` 삭제. i18n(`useLocale`) 서술은 실제 코드에 근거가 없어 정정 — 문구는 영문 하드코딩. 장치 힌트의 "In Use" 문구 삭제(`probed`가 참일 때만 채널 수 표시). 트리거 위치를 `dashboard/Sidebar.tsx`로 정정. 섹션 1·2·3·4·5 부분 갱신 (커밋 범위: 4d86f32..24d1daa)
