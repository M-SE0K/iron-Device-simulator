...
| `hooks/useMetricChartRuntime.ts` | React 상태로 필요한 최소값(현재값, 축 소수점 자리수, `showChart`)만 100ms 주기로 갱신하고 N11/N12 E2E 지연을 계측 |
...
DashboardClient → TemperatureChart/ExcursionChart(store=ChartStore)
  useMetricChartRuntime(store) → 100ms throttle 리드아웃 + N11/N12 계측
...
- **`TemperatureChart(props)`** / **`ExcursionChart(props)`** — `store: ChartStore`, `isActive`, `streaming?`, `audioDuration?`, `perfTrack?`, `onExpand?`를 공통으로 받습니다. `TemperatureChart`는 여기에 `warnThreshold?`/`dangerThreshold?`를 더 받습니다.
- **`useMetricChartRuntime({ metric, store, isActive, audioDuration?, perfTrack }): { current, timeDecimals, onRender, showChart }`** — 헤더 현재값과 축 소수점 자리수처럼 리렌더가 필요한 값만 뽑아줍니다.
