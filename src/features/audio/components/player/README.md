...
와이어 프레임 포맷은 인터리브 Int32(V I V I, `BYTES_PER_SAMPLE` = 4바이트)이고, 기본 설정(48 kHz, 480 samples/ch)에서 엔진에 보내는 분석 프레임 1개는 480 × 2ch × 4바이트 = **3840 bytes/frame**, 10 ms 분량이다. 네이티브 캡처는 Calibration의 채널 수(최대 8ch)만큼 장치 채널을 열지만 엔진에는 ch0/ch1만 보내고 나머지는 세션 버퍼에 보존한다. `sampleRate`/`bufferSize`는 Calibration UI 값이 세션 시작 시점에 `EngineRuntimeConfig`로 고정되므로 고정 상수가 아니다.
...
- `lib/engine/utils.ts` — `encodeToInt32()` (Float32 플래너 → Int32 인터리브, 웹 캡처 경로용).
