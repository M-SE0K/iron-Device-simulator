...
업로드한 파일을 재생하면서 실제 하드웨어가 되돌려 주는 ch0(V)/ch1(I) 센스 신호를 캡처해 WASM(WebAssembly) 분석 엔진에 흘려보내고, 엔진이 돌려주는 온도·익스커션 프레임을 대시보드로 전달하는 입력 계층이다. 개발자는 이 도메인만 보면 "소리가 어떻게 분석 프레임이 되는가"의 전 과정을 파악할 수 있다.

이 도메인은 업로드 PCM을 직접 분석하지 않는다. WaveSurfer로 파일을 재생해 출력 장치(앰프/스피커)로 내보내는 동시에, 그 하드웨어 응답을 ch0(V)/ch1(I)로 캡처해 **하나의 캡처 파이프라인**(`capture/useCaptureSession.ts`)에 흘려보낸다. 플레이어는 `createAnalysisSocket()`이 돌려주는 WebSocket 모양의 인프로세스 소켓(`LocalWasmSocket`)에 대고 쓰기 때문에, 코드가 WebSocket API 형태여도 데이터는 페이지 밖으로 나가지 않는다.
...
`DashboardClient.tsx`(`components/dashboard/`)가 파일 플레이어를 마운트하고, 플레이어가 올려보내는 `AnalysisFrame`(`onFrameReceived`)을 출력 큐/FIFO를 거쳐 Temperature/Excursion 차트로 렌더링한다. 즉 이 도메인은 프로젝트 데이터 흐름의 최상류(입력 → 분석 요청)를 담당하고, 하류(수신 프레임의 코얼레싱·이벤트 검출·차트)는 `dashboard/` + `lib/render/`가 담당한다.
...
외부에서 이 도메인을 import하는 파일은 `components/dashboard/DashboardClient.tsx`(파일 플레이어 + `WaveformPlayerHandle`) 한 곳이다.
...
| `capture/useCaptureSession.ts` | 캡처+분석+녹음 세션 훅. 분석 소켓(`createAnalysisSocket`)과 `onmessage`(ready/frame/error) 핸들링을 소유하고, 실제 캡처는 `window.audioCapture` 존재 시 `useNativeCapture`, 부재 시 `useWebAudioWorkletCapture`로 분기한다. 세션 동안의 전 채널 원본 PCM(`rawCaptureRef`)을 축적해 `getRecordedBlob()`으로 WAV 인코딩한다. `pauseRecording`/`resumeRecording`은 소켓/캡처를 유지한 채 분석 프레임 전송과 저장 버퍼 축적만 함께 멈춘다(재생 일시정지 시 WASM 온도 누적·차트 시간축 보존용). |
| `capture/build-init-message.ts` | 분석 소켓 `init` 메시지(JSON: `ampOutputPower`/`speakerModel`/`ambientTemp`/`sampleRate`/`bufferSize`) 빌더. |
| `capture/useNativeCapture.ts` | Electron CoreAudio 캡처 경로. `window.audioCapture.start()`로 상주 헬퍼를 띄우고 장치가 실제 적용한 `actual.sampleRate`/`actual.bufferSize`를 우선해 소켓 init에 반영한다("actual 우선" 원칙). `actual.bufferSize`(samplesPerCh)는 그대로 `ff_prot_start_exec`의 dt 계산 인자로 쓰이지만, `actual.sampleRate`는 와이어/세션 설정에만 쓰이고 엔진 호출 인자로는 전달되지 않는다(검증된 실제 벤더 시그니처엔 `sample_rate_hz`가 없음 — `native/wasm-engine/VENDOR-API-SPEC.md` 2.2절). 청크는 reframer를 거쳐 2ch(V/I) 분석 프레임과 전 채널 원본 프레임으로 분기된다. |
...
내부 처리 흐름 — 캡처 경로(useCaptureSession.start):
...
- `WaveformPlayerHandle` → `{ sendMessage(msg: object): void; pause(): void; exportRecordedAudio(): Blob | null; subscribeCaptureStream(fn: CaptureStreamListener): () => void }` → 상위(DashboardClient)가 ref로 제어하는 핸들. `pause()`는 캡처 세션을 닫지 않아(저장 버퍼만 멈춤) 재개 시 차트가 보존되고, `exportRecordedAudio()`는 원본 파일이 아니라 세션이 실제 캡처한 전 채널 PCM을 WAV로 반환한다(캡처된 적 없으면 null). `subscribeCaptureStream()`은 원본 캡처 청크 실시간 스트림 구독(구독 해제 함수 반환)으로, `ChartDetailOverlay` 채널 뷰가 이걸 구독해 폴링 없이 실시간 갱신된다.
- `useCaptureSession(deps: UseCaptureSessionDeps)` → `{ start, stop, cleanup, isRecording, micError, sampleRate, deviceName, actualBufferSize, actualLatency, getRecordedBlob, getProtectedBlob, hasProtectedRecording, sendMessage, pauseRecording, resumeRecording, subscribeCaptureStream, frameCountRef, framesRcvdRef }` → 캡처+분석+녹음 세션. `start()`는 `window.audioCapture` 존재 시 네이티브, 부재 시 getUserMedia 폴백을 고른다. `getRecordedBlob()`은 세션의 전 채널 버퍼를 동기적으로 N채널 인터리브 int16 WAV Blob(ch0=V, ch1=I, ch2.. 확장 채널)으로 반환하고, 캡처된 적이 없으면 null이다 — 저장 파이프라인은 호출자(`DashboardClient`)가 소유한다.
...
- 2026-07-28: 마이크 패널 제거 반영 — `MicrophonePlayer.tsx`를 삭제하고 이 도메인을 파일 재생 전용으로 좁혔다. 마이크 패널만 쓰던 세션 API(`saveRecording`/`hasRecording`/`saving`/`recordingChannels`/`onSaveRecording`)와 그 페이로드 타입 `CaptureRecordingExport`도 함께 없어져 저장은 `getRecordedBlob()`을 호출자가 직접 쓰는 한 갈래만 남았다. 재생 위치를 상위로 올리던 `onTimeUpdate` prop도 소비자가 사라져 `WaveformPlayer`/`DuplexFilePlayer`에서 제거했다(두 플레이어의 진행바용 로컬 `currentTime`은 유지). 섹션 1·2·3·4·5 부분 갱신 (커밋 범위: c1fe7eb..HEAD)
