...
- `lib/wav-decoder.ts` — `decodeAudioChannels(blob)`: RIFF 청크를 직접 파싱해 int16/int32/float32 PCM WAV를 채널별 평면 `Float32Array`([-1,1] 정규화)로 변환하고 WAV가 아니면(원본 mp3 저장 등) `decodeAudioData`로 폴백한다. `wav-encoder.ts`가 만든 N채널 int16 PCM WAV를 되읽는 것이 1차 목적이며, 지원 포맷은 헤더의 `bitsPerSample`을 따르므로 기존 32-bit 저장물도 그대로 읽힌다(디코더 소스에 명시).
...
- 2026-07-14: 저장 WAV의 PCM 샘플 폭이 int32에서 int16으로 바뀐 것을 반영 — `decodeAudioChannels`가 되읽는 `wav-encoder.ts` 산출물이 이제 N채널 int16 WAV다. 디코더는 헤더의 `bitsPerSample`을 따르므로 기존 32-bit 저장물도 그대로 읽힌다(int16/int32/float32 지원 유지). 섹션 4 부분 갱신 (커밋되지 않은 워크트리 변경 반영)
