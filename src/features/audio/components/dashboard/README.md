...
WaveformPlayer / DuplexFilePlayer
  ...
  → [requestAnimationFrame 스케줄러] detectEvents(bucket) + coalesceFrames(bucket)
...
- 2026-07-28: 출력 큐 배출을 고정 5 ms `setInterval`에서 `requestAnimationFrame`으로 변경. 데이터 수신 주기와 59.94/60/120 Hz 화면 주기의 위상차로 한 표시 프레임에 React 커밋 수가 달라지는 문제를 없애고, 실제 화면 표시 기회마다 최대 한 번만 차트를 갱신한다.
