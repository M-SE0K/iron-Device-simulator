...
빌드 파이프라인과의 관계: `npm run build:desktop|electron|mobile` → `scripts/build/build-static-local.sh`가 `page.tsx`를 백업(`page.tsx.bak`) 후 `dynamic` 값을 `"force-static"`으로 치환 → `MOBILE_BUILD=1 npx next build`로 `out/` 정적 export → 종료 시 원복. 정적 export에서는 `USE_QUEUE`가 빌드 시점 값으로 고정된다(기본 `true`).
