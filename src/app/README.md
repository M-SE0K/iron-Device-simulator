...
| `layout.tsx` | 루트 레이아웃. `globals.css` import, `metadata`(타이틀/설명), `viewport`(`viewportFit: "cover"` — 노치 영역까지 그려 `env(safe-area-inset-*)`가 실제 값을 갖게 함) export, `<html lang="en">` 아래 `TauriBridgeInit` → `IronPerfInit`을 가장 먼저 렌더한 뒤 `ErrorPopupProvider` → `ActiveDrawerProvider` → `CalibrationProvider` → `WorkspaceProvider` 중첩. 가장 안쪽에서 `SplashGate`가 `{children}`을 감싸 시작 로딩 화면을 얹는다. |
...
| `globals.css` | 전역 스타일. Pretendard(가변, jsdelivr CDN)·Google Fonts(Inter, JetBrains Mono) import, 로딩 화면 전용 `Exo 2` 가변 woff2 `@font-face` 2건(`/fonts/exo2-latin.woff2`, `/fonts/exo2-latin-ext.woff2`), Tailwind 3계층 지시자, CSS 변수 3종, `html { font-size: 87.5% }`(rem 기반 유틸 전역 축소), 커스텀 스크롤바(6px), `@layer components`의 `.card`/`.card-header`/`.card-title`/`.badge`(blue/green/red 변형), `@layer utilities`의 `expand-down`(0.22s)/`slide-in-right`(0.24s) 애니메이션을 정의한다. `Exo 2`만 CDN이 아니라 `public/fonts/`에 담아 함께 배포한다. 패키징된 데스크톱 앱은 네트워크가 없는 환경에서도 로딩 화면을 제대로 그려야 하기 때문이다. |
                ...
                └── ActiveDrawerProvider (@/features/audio/components/ActiveDrawerContext)
                      └── CalibrationProvider  (@/features/audio/components/calibration/CalibrationContext)
                            └── WorkspaceProvider (@/features/audio/components/workspace/WorkspaceContext)
                                  └── SplashGate (@/shared/components/splash/SplashGate)
                                        ├── {children} = page.tsx
                                        │     └── DashboardClient (@/features/audio/components/dashboard/DashboardClient, prop 없음)
                                        └── LoadingSplash (ENTER를 누르기 전까지 z-index 9999로 덮는 오버레이)
```
...
- `layout.tsx` — `export default function RootLayout({ children })`, `export const metadata: Metadata`, `export const viewport: Viewport`. `TauriBridgeInit` → `IronPerfInit`을 먼저 렌더한 뒤 `ErrorPopupProvider` → `ActiveDrawerProvider` → `CalibrationProvider` → `WorkspaceProvider` 순으로 앱 전역 Provider를 조립하고 가장 안쪽에서 `SplashGate`로 `{children}`을 감싼다.
...
- `globals.css` — `.card*`/`.badge*` 컴포넌트 클래스와 `.animate-expand-down`/`.animate-slide-in-right` 유틸리티: 하위 도메인 컴포넌트들이 공용으로 쓰는 전역 클래스. 여기에 `font-family: 'Exo 2'` `@font-face` 2건을 선언하므로 로딩 화면 밖에서 이 글꼴을 쓸 때도 별도 로딩 없이 이름만 지정하면 된다.
...
- 2026-08-20: 앱 시작 로딩 화면 도입을 반영했다. `layout.tsx`의 `WorkspaceProvider` 안쪽에 `SplashGate`(`@/shared/components/splash/SplashGate`)가 들어와 `{children}` 위에 오버레이를 겹친다. `globals.css`에는 로딩 화면 전용 `Exo 2` 가변 woff2 `@font-face` 2건이 추가됐다(`public/fonts/`에 로컬 번들 — 오프라인 데스크톱 앱 대응). 섹션 2·3·4·5 부분 갱신 (커밋 범위: b0db42d..HEAD, 작업 트리의 커밋되지 않은 변경 포함)
